...
class HumanDetector:
    def __init__(self, name="vitdet", device="cuda", **kwargs):
        ...
        if name == "vitdet":
            ...
            try:
                ...
                # Use provided path or default to yolov8l.pt
                model_path = kwargs.get("path", "yolov8l.pt")
                if not model_path or model_path == "":
                    model_path = "yolov8l.pt"
                
                self.detector = YOLO(model_path)
                self.detector_func = self.run_yolo
            except ImportError:
    ...
    def run_yolo(self, detector, img, bbox_thr=0.5, **kwargs):
        # YOLO expects BGR or RGB. It handles it.
        # img is BGR from cv2
        results = detector(img, verbose=False, classes=[0], conf=bbox_thr) # 0 is person
        boxes = []
        for r in results:
