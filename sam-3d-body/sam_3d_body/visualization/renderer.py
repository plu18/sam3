...
class Renderer:
    ...
    def __call__(
        ...
        Args:
            ...
            alpha (float): Transparency of the mesh (0.0 to 1.0).
        """
        ...
        # Check if mesh_base_color is an array of colors (per vertex)
        use_vertex_color = isinstance(mesh_base_color, (np.ndarray, list)) and len(mesh_base_color) == vertices.shape[0]
        if use_vertex_color:
            vertex_colors = np.array(mesh_base_color)
            # Convert to 0-255 uint8 if float 0-1
            ...
            if vertex_colors.shape[1] == 3:
                ...
                vertex_colors = np.hstack([vertex_colors, np.full((vertices.shape[0], 1), alpha_uint8, dtype=np.uint8)])
