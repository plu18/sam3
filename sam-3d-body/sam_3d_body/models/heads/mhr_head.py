...
class MHRHead(nn.Module):
    ...
    def _apply(self, fn):
        super()._apply(fn)
        # MHR must remain in float32 because sparse ops don't support BFloat16/FP16
        if hasattr(self, "mhr"):
            self.mhr.to(dtype=torch.float32)
        # keypoint_mapping is applied to MHR output (which is float32), so it must be float32
        if hasattr(self, "keypoint_mapping"):
            self.keypoint_mapping.data = self.keypoint_mapping.data.to(dtype=torch.float32)
        return self
    ...
    def replace_hands_in_pose(self, full_pose_params, hand_pose_params):
        ...
        # Drop it in
        full_pose_params[:, self.hand_joint_idxs_left] = left_hand_params_model_params.to(full_pose_params.dtype)
        full_pose_params[:, self.hand_joint_idxs_right] = right_hand_params_model_params.to(full_pose_params.dtype)
