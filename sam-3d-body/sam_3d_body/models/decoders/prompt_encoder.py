...
class PromptEncoder(nn.Module):
    ...
    def forward(
        ...
        # Fix for mixed precision: use correct dtype
        dtype = self.point_embeddings[0].weight.dtype
        sparse_embeddings = torch.empty(
            (bs, 0, self.embed_dim), device=self._get_device(), dtype=dtype
        )
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def forward(self, size: Tuple[int, int]) -> torch.Tensor:
        ...
        # Use the dtype of the registered buffer to ensure compatibility
