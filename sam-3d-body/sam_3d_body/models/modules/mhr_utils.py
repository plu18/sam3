...
def batchXYZfrom6D(poses):
    ...
    singular = sy < 1e-6
    singular = singular.to(matrix.dtype)
...
def compact_cont_to_model_params_hand(hand_cont):
    ...
    hand_model_params_threedofs = batchXYZfrom6D(hand_cont_threedofs).flatten(-2, -1).to(hand_cont.dtype)
    ## Next for 1DoFs
...
def compact_cont_to_model_params_body(body_pose_cont):
    ...
    ## First for 3dofs
    body_cont_3dofs = body_cont_3dofs.unflatten(-1, (-1, 6))
    body_params_3dofs = batchXYZfrom6D(body_cont_3dofs).flatten(-2, -1).to(body_pose_cont.dtype)
    ## Next for 1dofs
    body_cont_1dofs = body_cont_1dofs.unflatten(-1, (-1, 2))  # (sincos)
